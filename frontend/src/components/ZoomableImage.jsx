...
export default function ZoomableImage({ src, alt, className = "" }) {
  ...
  const handleWheel = (e) => {
    ...
    nextScale = Math.max(0.5, Math.min(nextScale, 4));
    setScale(nextScale);
    if (nextScale <= 1) {
      setOffset({ x: 0, y: 0 });
    }
  ...
  const handlePointerMove = (e) => {
    ...
    // Set a boundary limit based on scale (optional, but keep it loose for better UX)
  ...
  // Attach non-passive wheel event listener to avoid console warnings in modern browsers
  useEffect(() => {
