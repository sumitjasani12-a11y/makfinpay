...
async def main():
    ...
    sql = """
        ...
        WHERE h.status = 'ACTIVE' OR (
            ($1::timestamptz IS NULL OR h.activated_at <= $2::timestamptz)
            AND ($2::timestamptz IS NULL OR (h.deactivated_at IS NULL OR h.deactivated_at >= $1::timestamptz))
        )
