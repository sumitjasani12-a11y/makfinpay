...
async def main():
    ...
    sql = """
        ...
        WHERE h.activated_at <= $2::timestamptz AND (h.deactivated_at IS NULL OR h.deactivated_at >= $1::timestamptz)
    ...
    print(f"Total sessions active today ({from_dt.date()}): {len(rows)}\n")
